...
function renderMissileVisuals(
  ...
  ctx.fill();
  drawIcon(missileIcon, trajectoryPoint.x, trajectoryPoint.y, trajectoryPoint.heading, MISSILE_ICON_BASE_SCALE);
  ctx.restore();
}
 ...
 */
function handleMirvSplitting(missile: any): boolean {
...
function checkAndHandleInterception(
  ...
  const allies = nations.filter(n => n.treaties?.[missile.target.id]?.alliance && n.defense > 0);
...
function drawMissiles() {
  ...
  for (let i = S.missiles.length - 1; i >= 0; i--) {
    ...
    const trajectoryPoint = calculateMissileTrajectoryPoint(missile, startX, startY, targetX, targetY);
    renderMissileVisuals(missile, startX, startY, targetX, targetY, trajectoryPoint);
    ...
    if (missile.t >= 1 && !missile.hasExploded) {
      ...
      const mirvActivated = handleMirvSplitting(missile);

      // Priority 2: Check for interception (may remove missile)
      const wasIntercepted = checkAndHandleInterception(missile, i, targetX, targetY);
      if (wasIntercepted) {
      ...
      if (!mirvActivated) {
        ...
        explode(targetX, targetY, missile.target, missile.yield, missile.from || null,
