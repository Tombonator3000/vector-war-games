...
import { resolvePublicAssetPath } from './renderingUtils';

// Define the base leader image paths (without base URL prefix)
const leaderImagePaths: Record<string, string> = {
...
// Export resolved leader images with proper base URL handling
export const leaderImages: Record<string, string> = Object.fromEntries(
  Object.entries(leaderImagePaths).map(([name, path]) => [name, resolvePublicAssetPath(path)])
);
