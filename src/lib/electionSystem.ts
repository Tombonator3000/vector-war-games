...
const publicOpinionDriftMap = new Map<string, number>();
...
export function calculatePublicOpinion(
  ...
  const previousDrift = publicOpinionDriftMap.get(nation.id) ?? 0;
  ...
  publicOpinionDriftMap.set(nation.id, nextDrift);
